...
import org.hibernate.action.EntitiesUpdateAction;
...
public class ActionQueue {
	...
	/**
	 * Method to collect the similar updates.
	 * Assuming that similar kind of updates will be in sequence
	 * in the updates collection.
	 */
	private void optimizeCollectionUpdates(){
		//this is a collection of similar updates collections
		ArrayList similarUpdatesCollections = new ArrayList();		
		for(int i=0;i<updates.size()-1;i++){			
			EntityUpdateAction currentUpdateAction = (EntityUpdateAction)updates.get(i);
			EntityUpdateAction nextUpdateAction = (EntityUpdateAction)updates.get(i+1);
			if(currentUpdateAction.compareWithUpdateAction(nextUpdateAction)){
				//there may be similar updates of this kind
				//assuming similar kind of updates will be in sequence
				ArrayList similarUpdates = new ArrayList();
				similarUpdates.add(currentUpdateAction);
				similarUpdates.add(nextUpdateAction);
				int j=i+2;
				for(;j<updates.size();j++){
					nextUpdateAction = (EntityUpdateAction)updates.get(j);
					if(currentUpdateAction.compareWithUpdateAction(nextUpdateAction))
						similarUpdates.add(nextUpdateAction);
					else break;
				}
				//all similar updates are gathered
				i=j-1;
				similarUpdatesCollections.add(similarUpdates);				
			}
		}
		
		for(int i=0;i<similarUpdatesCollections.size();i++){
			ArrayList similarUpdates = (ArrayList)similarUpdatesCollections.get(i);
			updates.removeAll(similarUpdates);
			System.out.println("A collection of "+similarUpdates.size()+" similar updates found ");
			executeWithOneUpdateQuery(similarUpdates);
		}
	}
	
	private void executeWithOneUpdateQuery(ArrayList similarUpdates){
		EntityUpdateAction updateAction = (EntityUpdateAction)similarUpdates.get(0);
		new EntitiesUpdateAction( (Serializable)updateAction.getIdFromEntityAction(),
		        updateAction.getState(),
		        updateAction.getDirtyFields(),
		        updateAction.isHasDirtyCollection(),
		        updateAction.getPreviousState(),		       
		        updateAction.getInstanceFromEntityAction(),
		        updateAction.getRowId(),
		        updateAction.getPersisterFromEntityAction(),
		        session,
		        similarUpdates).execute();		
	}
